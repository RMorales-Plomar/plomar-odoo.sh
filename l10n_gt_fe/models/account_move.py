...
#dom is used to parse the xml response from the server
...
class AccountMove(models.Model):
    ...
    def action_post(self):
        ...
        if self.company_id.country_id.code == 'GT':
            for rec in self:
                if rec.move_type in ['out_invoice', 'out_refund']:
                    if (rec.partner_vat is False):
                        raise UserError(_('Error. El cliente no tiene NIT'))
                    if rec.partner_vat == 'CF' and rec.amount_total > 2500.00:
                        raise UserError(_('Error. Es necesario consignar el CUI del cliente %s') %(rec.partner_id.name))
    ...
    def _xml(self):
        ...
        #ET.SubElement(Adenda, 'ReferenciaDelCliente').text = self.fe_customer_reference
        ...
        #Fecha y Hora del DTE
        invoice_date = fields.Date.from_string(self.invoice_date).strftime('%Y-%m-%d')
        invoice_time = fields.Datetime.context_timestamp(self, timestamp=datetime.now()).strftime('%H:%M:%S')
        date_str = ""
        if invoice_date and invoice_time:
            date_str = (('%sT%s-06:00') %(invoice_date, invoice_time))
        DatosGenerales = {
            'CodigoMoneda': self.currency_id.name,
            'FechaHoraEmision': date_str,
            'Tipo': self.fe_type
        }
        ...
        direccion = '%s %s' % (self.partner_id.street, self.partner_id.street2)
        ...
        if self.fe_type != 'NABN':
            if self.fe_type == 'FESP':
                ...
                _logger.info('*************groups_by_subtotal***************************')
                _logger.info(groups_by_subtotal)
        ...
        _logger.info(fe)
        xml_string = ET.tostring(fe, encoding='UTF-8', method='xml')
        _logger.info(str(xml_string.decode('utf-8')))
    ...
    def cancel_dte(self):
        ...
        ET.SubElement(AnulacionDTE, 'dte:DatosGenerales', {'FechaEmisionDocumentoAnular': datetime(invd.year, invd.month, invd.day, 0, 0, 1).isoformat(), #"2020-03-04T00:00:00-06:00"
                                                            ...
                                                            'ID':"DatosAnulacion",
                                                            "IDReceptor": self.partner_id.vat.replace("-", ""),
    ...
    def _compute_totals(self):
        for line in self:
            ...
            if line.tax_ids:
                ...
                line.price_tax = sum( t.get('amount', 0.0) for t in taxes_res['taxes'] )
                line.price_discount = (line.quantity * (line.price_unit * (line.discount / 100.0)))
            else:
                line.price_total = line.price_subtotal = subtotal
                line.price_tax = 0.00
                line.price_discount = 0.00
